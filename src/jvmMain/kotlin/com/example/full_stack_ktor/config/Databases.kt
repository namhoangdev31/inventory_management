...
import com.example.full_stack_ktor.table.inventory_management.*
import com.example.full_stack_ktor.table.report.*
import com.example.full_stack_ktor.table.user.*
import com.example.full_stack_ktor.table.user.RoleTable
...
object DatabaseFactory {
    ...
    fun init(environment: ApplicationEnvironment) {
        ...
        transaction(dbMain) {
            ...
            SchemaUtils.createMissingTablesAndColumns(com.example.full_stack_ktor.table.inventory_management.RoleTable)
...
object DatabaseAuthFactory {
    ...
    fun init(environment: ApplicationEnvironment) {
        dbAuth = Database.connect(
            url = "jdbc:postgresql://${
                environment.config.property("postgres.host").getString()
            }:${
                environment.config.property("postgres.port").getString()
            }/${environment.config.property("postgres.database").getString()}?prepareThreshold=0",
            driver = environment.config.property("postgres.driver").getString(),
